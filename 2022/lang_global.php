<?php


if (!$is_secure) {

    exit(0);

}


$lang_global = [

    "ana_sayfa" => [

        "en" => "Home",

        "tr" => "Ana Sayfa"

    ],

    "program" => [

        "en" => "Schedule",

        "tr" => "Program"

    ],

    "gecmis_sempozyumlar" => [

        "en" => "Past Symposiums",

        "tr" => "Geçmiş Sempozyumlar"

    ],

    "organizasyon_semasi" => [

        "en" => "Organization Chart",

        "tr" => "Organizasyon Şeması"

    ],

    "ulasim" => [

        "en" => "Transportation",

        "tr" => "Ulaşım"

    ],

    "konaklama" => [

        "en" => "Accommodation",

        "tr" => "Konaklama"

    ],

    "bildiri_gonderimi" => [

        "en" => "Paper Submission",

        "tr" => "Bildiri Gönderimi"

    ],

    "yazim_kurallari" => [

        "en" => "Paper Guidelines",

        "tr" => "Yazım Kuralları"

    ],

    "tam_metin_yazim_kurallari" => [

        "en" => "Full-Text Guidelines",

        "tr" => "Tam Metin Yazım Kuralları"

    ],

    "ozet_kitapcik" => [

        "en" => "Abstracts Book",

        "tr" => "Özet Kitabı"

    ],

    "bildiri_sunum_kurallari" => [

        "en" => "Presentation Guidelines",

        "tr" => "Bildiri Sunum Kuralları"

    ],

    "genel_bilgiler" => [

        "en" => "General Information",

        "tr" => "Genel Bilgiler"

    ],

    "sempozyum" => [

        "en" => "Symposium",

        "tr" => "Sempozyum"

    ],

    "onemli_tarihler" => [

        "en" => "Important Dates",

        "tr" => "Önemli Tarihler"

    ],

    "hisarli_ahmet" => [

        "en" => "Hisarli Ahmet",

        "tr" => "Hisarlı Ahmet"

    ],

    "hisarli_ahmet_hakkinda" => [

        "en" => "About",

        "tr" => "Hakkında"

    ],

    "hisarli_ahmet_hakkinda_full" => [

        "en" => "About Hisarli Ahmet",

        "tr" => "Hisarlı Ahmet Hakkında"

    ],

    "hakkinda_bir_yorum" => [

        "en" => "A Comment on Him",

        "tr" => "Hakkında Bir Yorum"

    ],

    "hakkinda_bir_yorum_full" => [

        "en" => "A Comment on Hisarli Ahmet",

        "tr" => "Hisarlı Ahmet Hakkında Bir Yorum"

    ],

    "kendi_sesiyle" => [

        "en" => "The Voice of Hisarlı Ahmet",

        "tr" => "Kendi Sesi"

    ],

    "kendi_sesiyle_full" => [

        "en" => "The Voice of Hisarli Ahmet",

        "tr" => "Kendi Sesiyle Hisarli Ahmet"

    ],

    "fotograflarla_hisarli_ahmet" => [

        "en" => "Hisarli Ahmet in Photos",

        "tr" => "Albüm"

    ],

    "fotograflarla_hisarli_ahmet_full" => [

        "en" => "Hisarli Ahmet in Photos",

        "tr" => "Hisarlı Ahmet Albüm"

    ],

    "kurullar" => [

        "en" => "Committees",

        "tr" => "Kurullar"

    ],

    "onur_kurulu" => [

        "en" => "Honorary Board",

        "tr" => "Onur Kurulu"

    ],

    "yurutme_kurulu" => [

        "en" => "Executive Board",

        "tr" => "Yürütme Kurulu"

    ],

    "bilim_kurulu" => [

        "en" => "Scientific Committee",

        "tr" => "Bilim Kurulu"

    ],

    "duzenleme_kurulu" => [

        "en" => "Organizing Committee",

        "tr" => "Düzenleme Kurulu"

    ],

    "etkinlikler" => [

        "en" => "Activities",

        "tr" => "Etkinlikler"

    ],
    "takvim" => [

        "en" => "Activities Calendar",

        "tr" => "Etkinlikler Takvimi"

    ],

    "genclik_seminerleri" => [

        "en" => "Youth Seminars",

        "tr" => "Gençlik Seminerleri"

    ],

    "genclik_seminerleri-duyuru" => [

        "en" => "Announcement",

        "tr" => "Duyuru"

    ],


    "genclik_seminerleri-program" => [

        "en" => "Program",

        "tr" => "Program"

    ],

    "genclik_seminerleri-general" => [

        "en" => "General Information",

        "tr" => "Genel Bilgiler"

    ],

    "genclik_seminerleri-invited" => [

        "en" => "Invited Speaker",

        "tr" => "Davetli Konuşmacı"

    ],
    "genclik_seminerleri-kabul-edilenler" => [

        "en" => "Accepted Presentations",

        "tr" => "Kabul Edilen Bildiriler"

    ],

    "iletisim" => [

        "en" => "Contact",

        "tr" => "İletişim"

    ],

    "uluslararasi_hisarli_ahmet_sempozyumu" => [

        "en" => "12th International Hisarli Ahmet Symposium",

        "tr" => "12. Uluslararası Hisarlı Ahmet Sempozyumu"

    ],

    "muzik_ve_iklim" => [

        "en" => "'Music and Climate'",

        "tr" => "'Müzik ve İklim'"

    ],

    "tarih_ve_yer" => [

        "en" => "9 - 12 June 2022",

        "tr" => "9 - 12 Haziran 2022"

    ],

    "yer" => [

        "en" => "Kütahya Health Sciences University",

        "tr" => "Kütahya Sağlık Bilimleri Üniversitesi"

    ],

    "katılım_ücreti" => [

        "en" => "Participation Fee",

        "tr" => "Katılım Ücreti"

    ],

    "konserler" => [

        "en" => "Concerts",

        "tr" => "Konser"

    ],

    "konferans" => [

        "en" => "Conference",

        "tr" => "Konferans"

    ],

    "kabul edilenler" => [

        "en" => "Accepted Presentations",

        "tr" => "Kabul Edilen Bildiriler"

    ],

    "panel" => [

        "en" => "Panel/Round Table",

        "tr" => "Panel/Açık Oturum"

    ],

    "atolye_calismalari" => [

        "en" => "Workshop",

        "tr" => "Atölye Çalışması"

    ],
    "sergi" => [

        "en" => "Exhibition",

        "tr" => "Sergi"

    ],
    "soylesi" => [

        "en" => "Conversation",

        "tr" => "Söyleşi"

    ],

    "ozet_gonderim" => [

        "en" => "Abstract Submission",

        "tr" => "Özet Gönderim"

    ],

    "genclik_seminerleri-abstractguidelines" => [

        "en" => "ANNOUNCEMENTS",

        "tr" => "Özet Yazım Kuralları"

    ],
    "genclik_seminerleri-kurallar" => [

        "en" => "Rules and Notes",

        "tr" => "Kurallar ve Notlar"

    ],

    "duyurular" => [

        "en" => "ANNOUNCEMENTS",

        "tr" => "DUYURULAR"

    ],
    "cautionbar" => [

        "en" => '<a href="symposium-gallery.php"><span class="blink">The Photograps from The 12th Symposium</span></a>',

        "tr" => '<a href="symposium-gallery.php"><span class="blink">12. Sempozyumdan Kareler</span></a>'

    ]

];


?>