...
class DB
{
    ...
    public function log_db_errors($error, $query)
    {
        ...
        if (defined('SEND_ERRORS_TO')) {
            ...
            $headers .= 'From: xxx.com <system@' . $_SERVER['SERVER_NAME'] . '.com>' . "\r\n";
    ...
    public function unFilter($data)
    {
        if (!is_array($data)) {
            $data = html_entity_decode($data, ENT_QUOTES, 'UTF-8');
        } else {
            $data = array_map(array($this, 'unFilter'), $data);
        }
        return $data;
    }
    ...
    public function get_row_assoc($query, $object = false)
    {
        self::$counter++;
        $row = $this->link->query($query);
        if ($this->link->error) {
